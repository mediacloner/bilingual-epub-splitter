...
def process_dir(text_dir):
    """Process every .html/.xhtml file under text_dir (recursively).

    Returns (files_changed, pairs_split).
    """
    html_paths = []
    ...
    for path in sorted(html_paths):
        ...
        if n:
            total_files += 1
            total_pairs_split += n
            print(f"  {os.path.relpath(path, text_dir)}: split {n} pair(s)")
    return total_files, total_pairs_split
...
def process_epub(epub_path):
    """Extract an EPUB, split its paragraphs, and write a new '… (split).epub'.

    The original file is left untouched. Returns (files_changed, pairs_split).
    """
    ...
    try:
        ...
        total_files, total_pairs = process_dir(tmp_dir)
        if total_pairs:
            repack_epub(tmp_dir, out_path)
            print(f"\nWrote: {out_path}")
        else:
            print("\nNothing to split; no output written.")
        return total_files, total_pairs
    finally:
...
def main():
    if len(sys.argv) != 2:
        sys.exit("Usage: split_paragraphs_short.py <text-dir | book.epub>")
    target = sys.argv[1]
    if os.path.isdir(target):
        total_files, total_pairs = process_dir(target)
    elif os.path.isfile(target):
        total_files, total_pairs = process_epub(target)
    else:
        sys.exit(f"No such file or directory: {target}")
    print(f"\nTotal: {total_pairs} paragraph pairs split across {total_files} files.")
